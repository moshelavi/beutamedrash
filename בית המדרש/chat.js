...
async function checkNewMessagesFor(partnerEmail) {
    ...
    try {
        const { data } = await supabaseClient.from('chat_messages')
            .select('*')
            .or(`and(sender_email.eq.${partnerEmail},receiver_email.eq.${currentUser.email}),and(sender_email.eq.${currentUser.email},receiver_email.eq.${partnerEmail})`)
...
async function renderChatList(filter, tabEl, isBackgroundUpdate = false) {
    ...
    data.forEach(msg => {
        ...
        if (!partners.has(partner)) {
            ...
            else if (partner === 'admin@system') category = 'other';
    ...
    chats.forEach(chat => {
        ...
        const name = user ? user.name : (chat.email.startsWith('book:') ? chat.email.replace('book:', '') : (chat.email === 'admin@system' ? 'הנהלה' : chat.email.split('@')[0]));
        ...
        newHTML += `
            <div class="chat-list-item ${chat.unread ? 'unread' : ''}" onclick="loadChatIntoMainArea('${chat.email}', '${name.replace(/'/g, "\\'")}')">
                <div class="chat-list-item-content">
                <div style="font-weight:bold; display:flex; justify-content:space-between; align-items:center;">
                    <span style="display:flex; align-items:center;">${onlineHtml}${name}</span>
                    <span style="font-size:0.7rem; color:#94a3b8; flex-shrink:0;">${timeDisplay}</span>
                </div>
                <div style="font-size:0.85rem; color:#64748b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${chat.lastMsg}</div>
            </div>
            ${chat.unread ? '<div style="width:10px; height:10px; background:var(--primary); border-radius:50%;"></div>' : ''}
            </div>
        `;
    });
    }
...
function loadChatIntoMainArea(email, name) {
    ...
    const isSystem = email === 'admin@system';
    ...
    const chatHtml = `
        <div class="chat-header" style="border-radius: 12px 12px 0 0; display: flex; justify-content: space-between; align-items: center; padding: 15px; background: var(--primary); color: white;">
            <div style="display:flex; align-items:center; gap:5px;">
                ${isBook ? '<i class="fas fa-book"></i>' : `<span class="online-dot" id="online-main-${email}" style="background-color:#cbd5e1; width:10px; height:10px; border-radius:50%; display:inline-block;"></span>`}
                <span>${name}</span>
            </div>
            <div style="font-size:1.1rem; display:flex; gap:15px; align-items:center;">
                <i class="fas fa-ban" onclick="openReportModal('${email}')" title="דיווח וחסימה" style="${banStyle}"></i>
                <i class="fas fa-compress-alt" onclick="minimizeMainChat('${email}', '${name.replace(/'/g, "\\'")}')" title="מזער לחלון צף" style="cursor:pointer;"></i>
                <i class="fas fa-times" onclick="closeMainChat()" title="סגור" style="cursor:pointer;"></i>
            </div>
        </div>
        <div class="chat-body" style="border-radius: 0 0 12px 12px;">
            ...
            <div class="chat-footer">
                ...
                <button class="btn" style="width:auto; padding:0 10px;" onclick="sendMessage('${email}')">
                    <i class="fas fa-paper-plane"></i>
                </button>
            </div>
        </div>
    ...
    if (!isBook) {
        ...
        if (partner && partner.lastSeen && (new Date() - new Date(partner.lastSeen) < 5 * 60 * 1000)) {
            document.getElementById(`online-main-${email}`).style.backgroundColor = '#22c55e';
            document.getElementById(`online-main-${email}`).style.boxShadow = '0 0 5px #22c55e';
        }
    }
    markAsRead(email);
}
...
async function loadChatRating() {
    ...
    if (messages && messages.length > 0) {
        ...
        if(dashStat) dashStat.innerText = count || 0;
    } else {
        display.innerText = 0;
    }
}
...
function appendMessageToWindow(partnerEmail, text, type, id, timestamp, isRead = false, senderEmail = null) {
    ...
    // Add Reactions (Like/Dislike) for Book Chats
    if (partnerEmail.startsWith('book:') && id) {
        ...
        reactionsDiv.innerHTML = `
            ...
            <div class="msg-actions-menu" style="position:relative; display:inline-block;">
                ...
                <div class="msg-menu-dropdown">
                    <div class="msg-menu-item" onclick="event.stopPropagation(); replyToMessage('${partnerEmail}', '${senderName}', '${fullTextSafe}'); this.parentElement.classList.remove('active');"><i class="fas fa-reply"></i> ציטוט</div>
                    <div class="msg-menu-item" onclick="event.stopPropagation(); openThread('${id}', '${fullTextSafe}', '${partnerEmail}'); this.parentElement.classList.remove('active');"><i class="fas fa-comments"></i> שרשור</div>
