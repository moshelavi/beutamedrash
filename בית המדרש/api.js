...
async function syncGlobalData() {
    try {
        ...
        if (currentUser && goals.length > 0) {
            ...
            if (myCloudGoals.length > 0) {
                const cloudIds = new Set(myCloudGoals.map(g => g.id.toString()));
                const now = Date.now();

                // שמירת לימודים מקומיים שעדיין לא סונכרנו לענן (תיקון הבאג)
                const localOrphanGoals = userGoals.filter(g => !cloudIds.has(g.id.toString()));
                if (localOrphanGoals.length > 0) console.log(`Found ${localOrphanGoals.length} local goals to merge.`);

                const mappedCloudGoals = myCloudGoals.map(g => ({
                    id: g.id.toString(),
                    bookName: g.book_name,
                    totalUnits: g.total_units,
                    currentUnit: g.current_unit || 0,
                ...
                // מיזוג חכם: אם יש לימוד מקומי עם אותו ID, נשמור עליו אם הוא מעודכן יותר (או פשוט נשמור עליו כי המשתמש עובד עליו)
                // במקרה זה, נניח שהמקומי הוא הקובע בסשן הנוכחי
                const mergedGoals = mappedCloudGoals.map(cloudG => {
                    const localG = userGoals.find(l => l.id === cloudG.id);
                    if (localG) {
                        // אם המקומי מתקדם יותר או שווה, נשמור אותו
                        if (localG.currentUnit >= cloudG.currentUnit) return localG;
                    }
                    return cloudG;
                });

                userGoals = [...mergedGoals, ...localOrphanGoals];
